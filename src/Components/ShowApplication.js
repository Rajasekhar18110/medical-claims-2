...
function ShowApplication() {
    ...
    const [rslt, setrslt] = useState({ current_auth: "" })
    const [imgSrc, setImgSrc] = useState([])

    const getData = async () => {
        const email = currentUser.email

        const res2 = await fetch("/get_images_id/" + id, {
            method: "POST",
        ...
        const res = await fetch("/showApplicationId/" + id, {
            method: "POST",
    ...
    const getStatus = async () => {
        const res = await fetch("/showApplicationIdStatus/" + id, {
            method: "POST",
            body: JSON.stringify({ user_data }),
            headers: { "Content-Type": "application/json" },
        })
